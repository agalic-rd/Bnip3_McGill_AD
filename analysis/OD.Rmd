...
```{r fig.width = 4, fig.height = 7}
make_signif_boxplot(mod_od_gamma_tot, xaxis = "age", adjust = p_adjust)
```
...
make_signif_boxplot(mod_od_gamma_tot, xaxis = "condition", facet = "age", ncol = 4, max_points = Inf, adjust = p_adjust) |> 
...
```{r}
mod_od_gamma_pooled <- glmmTMB::glmmTMB(
  od_corrected ~ condition + (1 | rat / section),
  family = Gamma("log"),
  data = optical_density$soma,
  REML = TRUE
)
...
cli_h1("[Parameters]")
parameters(mod_od_gamma_pooled)
cli_h1("[Fitness]")
performance(mod_od_gamma_pooled)
```
...
```{r fig.width = 10}
# check_model(mod_od_gamma_pooled) # Slow
```

### Predictive checks

```{r}
mod_od_gamma_pooled_dharma <- DHARMa::simulateResiduals(mod_od_gamma_pooled, plot = FALSE, n = 300, seed = getOption("seed"))
mod_od_gamma_pooled_dharma_t <- t(mod_od_gamma_pooled_dharma$simulatedResponse)
```

```{r fig.width = 10}
ppc_plots(mod_od_gamma_pooled, simulations = mod_od_gamma_pooled_dharma_t, term = "condition")
```

```{r fig.width = 10}
ppc_stat_plots(mod_od_gamma_pooled, simulations = mod_od_gamma_pooled_dharma_t, term = "condition")
```

### Potential outliers

```{r}
get_model_based_outliers(optical_density$soma, mod_od_gamma_pooled, mod_od_gamma_pooled_dharma, "od_corrected")
```
...
```{r}
parameters(
  mod_od_gamma_pooled, exponentiate = should_exp(mod_od_gamma_pooled),
...
```{r}
car::Anova(mod_od_gamma_pooled, type = 3)
```
...
distribution_summary(get_data(mod_od_gamma_pooled), dvs = find_response(mod_od_gamma_pooled), between = "condition")

cli_h1("[Emmeans]")

emmeans(mod_od_gamma_pooled, specs = "condition", regrid = "response")
```

### Contrasts

```{r}
emmeans(mod_od_gamma_pooled, specs = "condition", type = "response") |> 
  contrast(method = "pairwise", adjust = "none", infer = TRUE)
```

```{r fig.width = 6, fig.height = 7}
make_signif_boxplot(mod_od_gamma_pooled, xaxis = "condition", max_points = Inf) |> 
  save_png("mod_od_gamma_pooled", width = 6, height = 7, display = TRUE)
```
...
make_signif_boxplot(mod_bnip3_gamma, xaxis = "age", adjust = p_adjust)
```
...
```{r fig.width = 14, fig.height = 6}
make_signif_boxplot(mod_bnip3_gamma, xaxis = "condition", facet = "age", ncol = 4, max_points = Inf, adjust = p_adjust) |> 
...
```{r fig.width = 14, fig.height = 6}
make_signif_boxplot(mod_bnip3_gamma, xaxis = "age", facet = "condition", ncol = 4, max_points = Inf, adjust = p_adjust) |> 
